...
description: "Hyvä Checkout-specific backwards compatibility layer for Magewire. Use when migrating Hyvä Checkout v1 (Livewire v2) components to Magewire v3, debugging the hyva-checkout-main auto-enable rule, or editing the JS BC shims in this module (wire:model rewriting, entangle live-by-default, deprecated hook/event re-triggering, component $wire proxy). Triggers: Hyvä Checkout BC, SupportHyvaCheckoutBackwardsCompatibility, hyva-checkout-main, makeComponentBackwardsCompatible, MagewireHyvaCheckout BC templates."
...
Hyvä Checkout v1 was built against Magewire v1 / Livewire v2. This module (`Magewirephp_MagewireHyvaCheckout`) ships the BC JS layer that lets those v1 components keep running on Magewire v3 without code changes.

**This skill is specific to the Hyvä Checkout module.** The underlying BC system is framework-level and theme-agnostic — see `magewire-backwards-compatibility` for the core trait, attribute, `memo.bc.enabled` flag, and v2→v3 behavior reference.
...
- A module-scoped Feature that auto-enables BC for any component rendered inside the `hyva-checkout-main` layout container
...
`Magewirephp\MagewireHyvaCheckout\Magewire\Features\SupportHyvaCheckoutBackwardsCompatibility\SupportHyvaCheckoutBackwardsCompatibility`
...
Registered via `src/etc/frontend/di.xml` alongside the other Magewire Features.

---

## JS Templates

Four PHTML files under `src/view/frontend/templates/magewire-features/support-hyva-checkout-backwards-compatibility/`:
...
| File | Purpose |
|------|---------|
| `src/Magewire/Features/SupportHyvaCheckoutBackwardsCompatibility/SupportHyvaCheckoutBackwardsCompatibility.php` | Hyvä Checkout-specific BC Feature — manages `memo.bc.enabled` and the `hyva-checkout-main` default |
| `src/etc/frontend/di.xml` | Feature registration |

### JavaScript (PHTML)

| File | Purpose |
|------|---------|
| `src/view/frontend/templates/magewire-features/support-hyva-checkout-backwards-compatibility/magewire-hooks.phtml` | Promise runner, deprecation warnings, hook replacement mapping |
| `src/view/frontend/templates/magewire-features/support-hyva-checkout-backwards-compatibility/magewire-events.phtml` | Deprecated event re-triggering, property aliases |
| `src/view/frontend/templates/magewire-features/support-hyva-checkout-backwards-compatibility/magewire-attributes.phtml` | wire:model directive migration |
| `src/view/frontend/templates/magewire-features/support-hyva-checkout-backwards-compatibility/magewire-components.phtml` | $wire proxy, entangle live default, component BC proxy |
...
3. Eventually, when no components in the checkout rely on BC, the Feature can be removed from `src/etc/frontend/di.xml`.